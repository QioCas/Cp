...
signed main() {
    ...
    for(int cas = 1; cas <= testcases; ++cas) {
        PRINT("--- Case #%d start ---", cas);
        LetHimCook<void>(cas);
        // cout << (LetHimCook<bool>(cas) ? "Yes\n" : "No\n");
        // cout << LetHimCook<ll>(cas) << "\n";
        // auto ans = LetHimCook<>(cas);

        PRINT("--- Case #%d end ---", cas);
        PRINT("time = %d", (double)clock() / CLOCKS_PER_SEC);
        PRINT("++++++++++++++++++++");
    }
}
